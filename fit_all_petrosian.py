...
dfid = pd.read_csv('z1_catalogue.csv', usecols=["ID"])
ID = dfid["ID"]
image_list1 = []

for i in range(len(ID)):
    image_list1.append(str(ID[i]) + "_F150W.fits")
    image_list1.append(str(ID[i]) + "_F444W.fits")
...
df2 = pd.DataFrame(np.zeros((642, 16)), columns = ['read', 'vmax', 'vmin', 'cut', 'plot', 'noise_8_sigma',
...
for i in range(1,643,2):
    df2.iat[i,0] = p3.read_image(image_list1[i]) #read
    ...
    try:
        ...
        data = pd.read_csv("z1_catalogue_fit.csv", usecols=["ID", "RA", "DEC", "redshift_50", "stellar_mass_50",
        ...
        df.to_csv('z1_catalogue_fit_test.csv', header=True, index=None, sep=',', mode='w')
