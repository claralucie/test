...
for i in range(len(ras)):
    nc1_poly, miri1_poly = plot_primer_pointing(ax, v2s[i], v3s[i], ras[i], decs[i], v3pa)
...
egs_in_ceers.to_csv(r'/Users/clarapollock/Desktop/JWST/ceers.csv', header=True, index=None, sep=',', mode='w')
...
"""    
for k in range(len(nircam_polys)):
    mini_nc_mask = np.repeat(False, cat.shape[0])
    
    for j in range(cat.shape[0]):
        in_poly = [p.contains_point(cat.loc[j, ["RA", "DEC"]].values) for p in nircam_polys[k]]
        #nc_mask[j] = np.max(in_poly)
    
        mini_nc_mask[j] = np.max(in_poly)
            
        if not j % 1000:
            print("Done:", j, "in pointings:", np.sum(mini_nc_mask))
        
        for i in range(len(mini_nc_mask)):
            if (mini_nc_mask[k]==True):
                nc_mask[k]=True
"""               
"""
    mask = np.zeros(cat.shape[0]).astype(bool)
    ...
    for j in range(cat.shape[0]):
        in_poly = [p.contains_point(cat.loc[j, ["RA", "DEC"]].values) for p in nc1_poly]

        mask[j] = np.max(in_poly)
        if not j % 1000:
            print("Done:", j, "in pointings:", np.sum(mask))
"""
"""
RA_list = cat["RA"]
DEC_list = cat["DEC"]

RA_red = nc_mask*RA_list
DEC_red = nc_mask*DEC_list
